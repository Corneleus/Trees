             ...
             implements BSTInterface<T>
{
  ...
  public int singleParentCount(BinarySearchTree<T> tree){//8.23
	 return suchWow(tree.root);
  }  
  
  public int leafCount(BinarySearchTree<T> tree){//8.22
		return leafMeAlone(tree.root);
	}
  
  // helper method
  private int leafMeAlone(BSTNode root){//actual 8.22
	  BSTNode n = root;
	  ...
	  if(n == null){
		...
		return leafMeAlone(n.getLeft()) + leafMeAlone(n.getRight());	
  }
 
  private int suchWow(BSTNode root){// actual 8.23
	  BSTNode<T> g = root;
	  
	  if(g == null){
		  return 0;
	  }
	  if((g.getLeft() != null && g.getRight() == null) || (g.getLeft() == null && g.getRight() != null)){
		  return suchWow(g.getLeft()) + suchWow(g.getRight()) +1;
	  }
	  else
		  return suchWow(g.getLeft()) + suchWow(g.getRight());
  }
}
